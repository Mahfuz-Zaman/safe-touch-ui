...
export class VendorService {
  ...
  vendorRegistration(regData: Vendor) {
    return this.httpClient.post<{ vendorId: string, message: string }>(API_URL_VENDOR + 'registration', regData);
  }

  // For Login User..
  vendorLogin(data: any) {

    this.httpClient.put<{ token: string, expiredIn: number }>(API_URL_VENDOR + 'login', data)
      .subscribe(response => {
...
// For Save Token on Browser Storage..
  ...
  checkAndGetVendorByPhone(phoneNo: string) {
    return this.httpClient.get<{ data: boolean, message: string }>(API_URL_VENDOR + 'check-vendor-by-phone/' + phoneNo);
  }
